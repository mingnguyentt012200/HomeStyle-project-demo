...
| password | String | Yes | Min 8, max 128 characters. Must contain ≥1 uppercase, ≥1 lowercase, ≥1 number. Checked client-side in real time and server-side on submit. | 'Password must be at least 8 characters' / 'Password must include an uppercase letter, lowercase letter, and number' |
| confirmPassword | String | Yes | Must exactly match the password field value. Checked client-side on blur. Not sent to server. | 'Passwords do not match' |
| acceptedTerms | Boolean | Yes | Must be true. Stored with `accepted_at` timestamp and `terms_version` string. | 'You must accept the Terms & Conditions to create an account' |
...
| Password — rules not yet met | Password strength indicator (bar + checklist) | Checklist: '✓ 8+ characters', '✓ Uppercase', '✓ Number' | Incomplete rules in amber, met rules in green | Submit button disabled until all green |
...
| Form submitted successfully | Full-page confirmation screen | 'Check your email — we sent a verification link to [email]' | Clean confirmation layout | 'Resend email' link shown (rate-limited) |
...
[Figma — Registration Screen](https://www.figma.com/design/AaDMTC6h17acKOp4Jv5Vem/Homestyle?node-id=52-11706&t=RY4c3sYwtrRTkQLG-1)
