...
| # | Criteria | Test Type |
|---|---|---|
| AC-01 | Valid credentials return 200 with httpOnly access token and refresh token cookies. | Integration |
| AC-02 | Invalid credentials return 401 with generic message — no indication of which field is wrong. | Unit |
| AC-03 | After TC-10: 5 consecutive failures, account locked — returns 423 with countdown duration. | Integration |
| AC-04 | Lockout counter persists across server restarts (Redis-backed). | Integration |
| AC-05 | Access token not accessible via document.cookie (httpOnly verified). | Security |

---

#### US-02: Social OAuth login (Google)

As a registered customer, I want to log in with my Google account so
that I can access HomeStyle without managing a separate password.

**Acceptance Criteria**

| # | Criteria | Test Type |
|---|---|---|
| AC-01 | Google OAuth flow completes and issues JWT tokens for new and existing users. | E2E |
| AC-02 | OAuth state token mismatch returns 400 and logs security event. | Integration |
| AC-03 | Social login using an email already registered via email/password links the provider — no duplicate account created. | Integration |
| AC-04 | OAuth state token expires after TC-08: 10 minutes — stale callback returns error. | Integration |
