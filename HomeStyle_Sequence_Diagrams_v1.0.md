# HomeStyle — Sequence Diagrams

| | |
|---|---|
| **Document Type** | Sequence Diagram Catalogue (UML 2.x) |
| **Version** | 1.0 (Draft) |
| **Status** | Draft — for Review |
| **Author** | Business Analyst |
| **Last Updated** | 2026-05-10 |
| **Derived From** | BRD v4.0, Feature Specs v2.1 (F-01.x – F-10.x) |
| **Stakeholders** | Engineering, Product, QA, UX, Trade Operations, Compliance |

---

## 1. Purpose

This document captures every sequence-worthy interaction in the HomeStyle platform as Mermaid sequence diagrams. Each diagram complements its corresponding feature spec (`F-XX.X`) by visualising message ordering, timing, async behaviour, and inter-system contracts that are difficult to express in prose or tables alone.

Diagrams are organised by module (M01 – M10) and use the canonical participant names defined in Section 3 so they can be cross-referenced.

## 2. Scope & coverage

| Module | Diagrams |
|---|---|
| M01 — Authentication | 6 |
| M02 — Product Catalogue | 4 |
| M03 — Cart & Wishlist | 3 |
| M04 — Checkout & Payment | 4 |
| M05 — Order Management | 3 |
| M06 — Returns & Refunds | 3 |
| M07 — Reviews & Notifications | 4 |
| M08 — Compliance & Data | 3 |
| M09 — Admin Panel | 4 |
| M10 — Trade Portal | 3 |
| **Total** | **37** |

## 3. Canonical participants

| Alias | Display | Role |
|---|---|---|
| `G` | Guest | Unauthenticated visitor |
| `C` | Customer | Registered Customer |
| `T` | Trade | Verified Trade Account |
| `AOps` | Admin-Ops | Admin — Operations |
| `ACont` | Admin-Content | Admin — Content |
| `ATra` | Admin-Trade | Admin — Trade |
| `SA` | Super Admin | Super Admin |
| `FE` | Frontend (Next.js SSR + CSR) | Storefront / Admin Panel |
| `API` | Backend API | REST service layer |
| `DB` | PostgreSQL | Primary data store |
| `R` | Redis | Sessions, holds, rate limits, cache |
| `S3` | AWS S3 | Object storage (assets, PDFs) |
| `BMQ` | BullMQ | Async job queue |
| `ES` | Elasticsearch / Algolia | Search index |
| `SG` | SendGrid / SES | Transactional email gateway |
| `Stripe` | Stripe | Payments & refunds |
| `OAuth` | Google / Facebook OAuth | Social identity provider |
| `Carrier` | Carrier API | UPS / FedEx / DHL / DPD |
| `WG` | White-Glove Partner | White-glove logistics partner |
| `VIES` | EU VIES | EU VAT Information Exchange System |
| `Tax` | Stripe Tax / TaxJar | Tax calculation API |
| `Audit` | Audit Log | Append-only audit store |

## 4. Quick TC reference (system-level timing constraints)

| TC | Value | Used in |
|---|---|---|
| TC-01 | 15-min customer JWT | F-01.2, F-01.5 |
| TC-02 | 30-day rotating customer refresh | F-01.2, F-01.5 |
| TC-03 | 15-min admin JWT | F-01.4, F-01.5 |
| TC-04 | 8-hr admin refresh | F-01.4, F-01.5 |
| TC-05 | 30-min admin inactivity (5-min warning) | F-01.4 |
| TC-06 | 24-hr email verification link, 3 resends/hr | F-01.1, F-01.6 |
| TC-07 | 1-hr password reset link, single-use | F-01.3 |
| TC-08 | 10-min OAuth state token | F-01.2 |
| TC-09 | 15-min S3 presigned URL (spec PDF) | F-02.3 |
| TC-10 | 5 attempts → 15-min customer lockout | F-01.2 |
| TC-11 | 3 attempts → 30-min admin lockout | F-01.4 |
| TC-12 | 30-day guest cart Redis TTL | F-03.1, F-03.2 |
| TC-13 | 15-min Redis inventory hold (SETNX) | F-04.1, F-04.2 |
| TC-14 | 30-min checkout session hard expiry | F-04.1, F-04.2 |
| TC-15 | 30-sec Stripe API timeout | F-04.2 |
| TC-16 | 24-hr Stripe idempotency key | F-04.2 |
| TC-17 | 4-hr order confirmation SLA | F-05.1, F-05.3 |
| TC-18 | 48-hr dispatch SLA | F-05.2, F-05.3 |
| TC-19 | 30-day return window | F-06.1 |
| TC-20 | Ship-back deadline | F-06.1 |
| TC-21 | 2-business-day admin return review SLA | F-06.2 |
| TC-22 | 1-business-day refund processing | F-06.3 |
| TC-23 | ≤12-business-day refund resolution | F-06.3, F-06.4 |
| TC-24 | 7-day post-delivery review prompt | F-07.1 |
| TC-25 | 180-day review submission window | F-07.1 |
| TC-26 | 48-hr review moderation SLA | F-07.2 |
| TC-27 | Low-stock alert threshold | F-07.4, F-09.3 |
| TC-28 | 1-hr back-in-stock notification (BullMQ) | F-03.3, F-09.3 |
| TC-29 | Email retry: 3× @ 1 min + 1× @ 15 min | F-07.3 |
| TC-30 | 24-hr Stripe webhook silence alert | F-06.3 |
| TC-31 | 30-day unverified account purge | F-08.3 |
| TC-32 | 7-year financial record retention | F-08.2, F-08.3 |
| TC-33 | 90-day server log purge | F-08.3 |
| TC-34 | 30-day GDPR erasure SLA | F-08.2 |
| TC-35 | 3-year audit log archive | F-08.3 |
| TC-36 | Cookie consent required + 12-month validity | F-08.1 |
| TC-37 | CCPA opt-out — immediate | F-08.1 |
| TC-TRA-01 | 5-business-day trade application SLA | F-10.1 |
...
**Maps to:** `F-01.1` · **Triggers:** TC-06 (verification link), TC-29 (email retry)
...
**Maps to:** `F-01.2` · **Triggers:** TC-01, TC-02, TC-08, TC-10
...
**Maps to:** `F-01.3` · **Triggers:** TC-07 (1-hr reset link), revokes all TC-02 refresh tokens
...
**Maps to:** `F-01.4` · **Triggers:** TC-03, TC-04, TC-05, TC-11
...
**Maps to:** `F-01.5` · **Triggers:** TC-01/03 access token rotation, family revocation on reuse
...
**Maps to:** `F-01.6` · **Triggers:** TC-06 (24-hr verification link)
...
**Maps to:** `F-02.2` · **Index sync:** ≤60 seconds after product publish
...
**Maps to:** `F-02.3` · **Triggers:** TC-09 (15-min S3 presigned URL)
...
**Maps to:** `F-02.4` · **Cache:** browser sessionStorage 1h per (zip, productId)
...
**Maps to:** `F-02.5` · **Search index sync within 60 s**
...
**Maps to:** `F-03.1` · **Triggers:** TC-12 (30-day Redis TTL for guest cart)
...
**Maps to:** `F-03.2` · **Atomic merge — all-or-nothing**
...
**Maps to:** `F-03.3` · **Triggers:** TC-28 (1-hr back-in-stock email via BullMQ hourly job)
...
**Maps to:** `F-04.1` · **Highest revenue-risk diagram in the system**
...
**Maps to:** `F-04.2` · **Triggers:** TC-15 (30-sec timeout), TC-16 (24-hr idempotency)
...
**Maps to:** `F-04.3` · **Double validation pattern**
...
**Maps to:** `F-04.4` · **Stripe Tax authoritative; flat-rate fallback on timeout**
...
**Maps to:** `F-05.1` · **Triggers:** TC-17 (4-hr SLA), TC-29 (email retry)
...
**Maps to:** `F-05.2` · **Tracking refresh every 2h via BullMQ**
...
**Maps to:** `F-05.3`, `F-05.4` · **Triggers:** TC-17, TC-18, TC-29
...
**Maps to:** `F-06.1` · **Triggers:** TC-19 (return window), TC-21 (admin SLA)
...
**Maps to:** `F-06.2` · **TC-21: 2-business-day review SLA**
...
**Maps to:** `F-06.3` · **Triggers:** TC-22, TC-23, TC-30
...
**Maps to:** `F-07.1` · **Triggers:** TC-24 (7-day prompt), TC-25 (180-day window)
...
**Maps to:** `F-07.2` · **TC-26: 48-hr SLA, auto-approve at 72h**
...
**Maps to:** `F-07.3` · **Common pipeline used by every customer-/admin-facing email**
...
**Maps to:** `F-07.4` · **Aggregates events across modules**
...
**Maps to:** `F-08.1` · **Triggers:** TC-36 (consent required), TC-37 (CCPA immediate)
...
**Maps to:** `F-08.2` · **Triggers:** TC-34 (30-day SLA), TC-32 (7-yr financial retention)
...
**Maps to:** `F-08.3` · **Triggers:** TC-31, TC-32, TC-33, TC-35
...
**Maps to:** `F-09.2` · **Step-up MFA for PII reveal; session revoke on deactivation**
...
**Maps to:** `F-09.3` · **Async job pattern; alerts on threshold cross**
...
**Maps to:** `F-09.4` · **Role-scoped cards, 60-s cache TTL**
...
**Maps to:** `F-09.5` · **CSV bulk import; trade-only invisible to consumer roles**
...
**Maps to:** `F-10.1` · **Triggers:** TC-TRA-01 (5-business-day SLA), provisions tier (F-10.2) + AM (F-10.3)
...
**Maps to:** `F-10.2` · **Server-side resolution; never client-side**
...
**Maps to:** `F-10.3` · **Per-row transactional, not all-or-nothing**
...
| Symbol | Meaning |
|---|---|
| `actor X` | Human role (Customer, Admin, etc.) |
| `participant X` | System or service |
| `->>` | Synchronous request / outbound message |
| `-->>` | Response / async acknowledgement |
| `Note over X,Y: …` | Annotation spanning participants — typically explains TC, BR, or business intent |
| `alt … else … end` | Branching (success / failure / variant paths) |
| `loop … end` | Repetition (poll, retry, per-row processing) |
| `par … and … end` | Parallel execution (fan-out across systems) |
| `rect rgb(r,g,b) … end` | Visual grouping (phase or sub-flow) |

# Appendix B — Cross-Module Touch Points

The following pipelines are referenced by many sequence diagrams and only diagrammed once:

| Pipeline | Diagram | Used by |
|---|---|---|
| Token rotation & refresh | SD-01.5 | All authenticated endpoints |
| Transactional email + TC-29 retry | SD-07.3 | F-01.1, F-01.3, F-01.6, F-03.3, F-05.1, F-05.3, F-06.1, F-06.2, F-06.3, F-07.1, F-07.2, F-09.2, F-10.1, F-10.3 |
| Admin alert generation | SD-07.4 | F-03.3, F-05.3, F-06.3, F-07.3, F-09.3, F-10.1 |
| Stripe webhook receiver | SD-04.2, SD-06.3 | All payment & refund flows |

# Appendix C — Open Questions

| ID | Question | Owner |
|---|---|---|
| OQ-01 | Confirm cache invalidation propagation latency for trade pricing (target ≤60 s) — affects SD-10.2 | Engineering |
| OQ-02 | Should cart-merge collisions notify the user *which* lines were capped, or just total? — affects SD-03.2 | Product |
| OQ-03 | OD-04 discount stacking final decision (currently Option A: one code per order) — affects SD-04.3 | Product Owner |
| OQ-04 | OD-08 made-to-order returns — confirm exchange/credit-only policy — affects SD-06.1, SD-06.2 | Legal + Trade Ops |
| OQ-05 | Tax API timeout fallback rate source of truth — affects SD-04.4 | Finance |
| OQ-06 | Trade application document retention — confirm 7-yr alignment with regional financial law — affects SD-10.1 | Legal |

# Appendix D — Assumptions

1. All HTTPS endpoints terminate at the API gateway; transport security is implied and not drawn.
2. JWT validation middleware runs on every authenticated endpoint; it is omitted from each diagram for clarity.
3. Audit log writes are best-effort async; failures do not block the user-facing transaction.
4. BullMQ workers are horizontally scaled; per-job ordering across workers is not assumed.
5. PostgreSQL transactions use `READ COMMITTED` isolation unless `FOR UPDATE` is shown explicitly.
6. Currency conversion (display vs. settlement) follows the rate captured at checkout-session creation (TC-14 lock).

---

*End of document — HomeStyle Sequence Diagrams v1.0*
